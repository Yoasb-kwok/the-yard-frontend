...
export default function OTPVerificationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // After success, navigate to reset password page or show success message
      setTimeout(() => {
        navigate('/login', { state: { message: t('otpVerification.success') } });
      }, 2000);
    } catch (err: unknown) {
      if (err instanceof Error) {
        setError(err.message);
      } else {
        setError(t('otpVerification.verificationFailed'));
      }
    } finally {
      setLoading(false);
    }
